...
test('@Webst Client App login', async ({ page }) => {
   ...
   await page.waitForLoadState('networkidle');
    ...
    await page.locator("div input[name='coupon']").fill('rahulshettyacademy');
    await page.locator("button[type='submit']").click();
    await page.waitForLoadState('networkidle');
    const couponApplied = await expect(page.locator("p[style*='green'].mt-1")).toHaveText('* Coupon Applied');
    console.log(couponApplied);
   await page.locator('input[type="text"].txt').nth(4).fill('123');
   await page.locator('input[type="text"].text-validated').nth(1).fill('anshika@gmail.com');
